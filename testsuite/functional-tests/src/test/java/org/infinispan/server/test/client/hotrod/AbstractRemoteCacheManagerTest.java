...
public abstract class AbstractRemoteCacheManagerTest {
   ...
   /*
    * Tests the configuration defaults (can be found in RemoteCacheManager class comment)
    */
   @Test
   ...
   public void testGetNonExistentCache() {
      ...
      // but it is not able to get some stats because it is not confugured properly
      RemoteCacheManager rcm = new RemoteCacheManager(createRemoteCacheManagerConfigurationBuilder().build());
   ...
   private void doTestDefaultLoadBalanding() throws Exception {
      ...
      if (!isLocalMode()) {
         ...
         assertTrue("LOCAL mode - loadbalancing server sequence expected " + expectedSequenceLocalMode
                          + ", actual sequence: " + serverAddrSequence.toString(),
                    serverAddrSequence.toString().equals(expectedSequenceLocalMode));
      }
   }
   ...
   private void doTestCustomLoadBalancing() throws Exception {
      ...
      sock_addr = (InetSocketAddress) tt.getServerAddress();
      ttf.releaseTransport(tt);
      assertTrue("load balancing first request: server address expected " + hostport0 + ", actual server address "
                       + sock_addr, sock_addr.equals(hostport0));

      tt = (TcpTransport) ttf.getTransport();
      sock_addr = (InetSocketAddress) tt.getServerAddress();
      ttf.releaseTransport(tt);
      assertTrue("load balancing second request: server address expected " + hostport0 + ", actual server address"
                       + sock_addr, sock_addr.equals(hostport0));
   }
   ...
   /*
   * Compares the provided configuration with properties obtained by reflection from a RemoteCache instance - this is
   * the most accurate view of properties being used by a RemoteCache
   */
   ...
   /*
    * request_balancing_strategy - this gets used directly by the TcpTransportfactory when processing HotRodOperations
    */
   ...
   /*
    * server_list - this gets used directly by the TcpTransportfactory when processing HotRodOperations
    */
...
//            serverList.append(serverAddress);
   ...
   /*
    * force_return_values - this gets used directly by the TcpTransportfactory when processing HotRodOperations
    */
   ...
   /*
    * tcp_no_delay - this gets used directly by the TcpTransportfactory when processing HotRodOperations
    */
   ...
   /*
    * transport_factory - used by OperationsFactory to provide each HotRod operation with its own transport
    */
   ...
   /*
    * marshaller - used by RemoteCacheImpl to marshal parameters for HotRod operations before sending over the TcpTransport
    */
   ...
   /*
    * hash_function_impl - used by TcpTransport to define a ConsistentHash, used in identifying a server to send a request to -
   ...
   /*
    * key_size_estimate - used by RemoteCacheImpl when marshalling keys for HotRod operations
    */
   ...
   /*
    * value_size_estimate - used by RemoteCacheImpl when marshalling values for HotRod operations
    */
    ...
    *
    * When RemoteCacheManager and RemoteCache are initialised, properties are set in a variety of classes These methods use
    * reflection to obtain the value of each property where it has effect.
    */
    /*
