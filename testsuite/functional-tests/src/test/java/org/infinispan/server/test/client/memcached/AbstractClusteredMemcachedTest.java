...
public abstract class AbstractClusteredMemcachedTest {
   ...
   public void setUp() throws Exception {
      mc1 = new MemcachedHelper(ENCODING, getServers().get(0).getMemcachedEndpoint().getInetAddress()
            .getHostName(), getServers().get(0).getMemcachedEndpoint().getPort(), 10000); // to run against
      // infinispan
      // memcached
      // server
      mc2 = new MemcachedHelper(ENCODING, getServers().get(1).getMemcachedEndpoint().getInetAddress()
